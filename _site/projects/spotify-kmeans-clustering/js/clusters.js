...
var skc_svg = d3.select('#d3-spotify-clusters');

var skc_margin = {top: 39, right: 12, bottom: 33, left: 40},
    skc_width  = $('#d3-spotify-clusters').width() - skc_margin.left - skc_margin.right,
    skc_height = $('#d3-spotify-clusters').height() - skc_margin.top - skc_margin.bottom;
...
function render_chart() {
    ...
    skc_svg.append("g")
        ...
        .attr("transform", `translate(${skc_margin.left}, ${skc_margin.top + oscars_height})`)
    ...
    skc_svg.append("g")
        ...
        .attr("transform", `translate(${skc_margin.left}, ${skc_margin.top})`)
    ...
    skc_svg.append('text')
        ...
        .attr('x', skc_margin.left + oscars_x(5e8))
        .attr('y', skc_margin.top + oscars_height + skc_margin.bottom - 3)
        .text("Box Office");

    skc_svg.append('text')
        .classed("oscars-axis-label", true)
        .attr("id", "oscars-y-axis-label")
        .attr('x', skc_margin.left / 2)
        .attr('y', skc_margin.top - 13)
        .text("Rating");
    ...
    skc_svg.append('text')
        ...
        .attr('x', skc_margin.left + skc_width / 2)
        .attr('y', skc_margin.top - (skc_width < 250 ? 25 : 27))
        .style('font-size', skc_width < 250 ? '12px' : '16px')
        .text("");
    ...
    skc_svg.append('text')
        ...
        .attr('x', skc_margin.left + skc_width / 2)
        .attr('y', skc_margin.top - 12)
        .style('font-size', skc_width < 250 ? '12px' : '16px')
        .text("Best Picture Nominees and Top Grossing Films, 2008 - 2017");

    if (skc_width < 340) {
        skc_svg.select('#oscars-title1').text("Best Picture Nominees and");
        skc_svg.select('#oscars-title2').text("Top Grossing Films, 2008 - 2018");
    } else if (skc_width < 430) {
        skc_svg.select('#oscars-title1').text("Best Picture Nominees and Top Grossing Films");
        skc_svg.select('#oscars-title2').text("2008 - 2018");
    }
}

function draw_dots(d) {
    skc_svg.selectAll('.oscars-dot')
        .data(d)
        .enter().append('circle')
            ...
            .attr('r', 2.5)
            .attr('cx', function(d) { return skc_margin.left + oscars_x(d.box_office); })
            .attr('cy', function(d) { return skc_margin.top + oscars_y(d.mean_rating); })
...
function draw_svm() {
    ...
    skc_svg.append('line')
        .attr('id', 'oscars-svm')
        .attr('x1', skc_margin.left)
        .attr('x2', skc_margin.left + oscars_x(2.75e8))
        .attr('y1', skc_margin.top + oscars_y(0.44))
        .attr('y2', skc_margin.top)
    ...
    skc_svg.append('text')
        ...
        .attr('x', skc_margin.left + oscars_x(2.75e8))
        .attr('y', skc_margin.top + 15)
        .text('SVM Line')
        .style('display', showing.svm ? 'initial' : 'none')
        .style('opacity', showing.svm ? 1 : 0);
    ...
    skc_svg.selectAll(".oscars-svm-region")
        .data(polygons)
        .enter().append("polygon")
            ...
            .attr("points", function(d) { 
                return d.map(function(d) {
                    var pts = [skc_margin.left + oscars_x(d.x), skc_margin.top + oscars_y(d.y)];
                    return pts.join(",");
                }).join(" ");
            })
...
function draw_better_svm() {
    ...
    skc_svg.append('line')
        ...
        .attr('x1', skc_margin.left)
        .attr('x2', skc_margin.left + skc_width)
        .attr('y1', skc_margin.top + oscars_y(0.8))
        .attr('y2', skc_margin.top + oscars_y(0.8))
    ...
    skc_svg.append('text')
        ...
        .attr('x', skc_margin.left + skc_width - 5)
        .attr('y', skc_margin.top + oscars_y(0.8) + 15)
    ...
    skc_svg.selectAll('.oscars-better-svm-region')
        .data(polygons)
        .enter().append("polygon")
...
function toggle_dots(animate) {
    ...
    animate_toggle_object(skc_svg.selectAll('.oscars-dot').filter(function() { return !d3.select(this).classed('2018'); }), showing.dots, animate, 500);
}

function toggle_dots2018(animate) {
    showing.dots2018 = !showing.dots2018;
    animate_toggle_object(skc_svg.selectAll('.oscars-dot').filter(function() { return  d3.select(this).classed('2018'); }), showing.dots2018, animate, 500);
}

function toggle_svm(animate) {
    showing.svm = !showing.svm;
    animate_toggle_object(skc_svg.selectAll('#oscars-svm, #oscars-svm-label'), showing.svm, animate, 500);
}

function toggle_svm_regions(animate) {
    showing.svm_regions = !showing.svm_regions;
    animate_toggle_object(skc_svg.selectAll('.oscars-svm-region'), showing.svm_regions, animate, 500);
}

function toggle_better_svm(animate) {
    showing.better_svm = !showing.better_svm;
    animate_toggle_object(skc_svg.selectAll('#oscars-better-svm, #oscars-better-svm-label'), showing.better_svm, animate, 500);
}

function toggle_better_svm_regions(animate) {
    showing.better_svm_regions = !showing.better_svm_regions;
    animate_toggle_object(skc_svg.selectAll('.oscars-better-svm-region'), showing.better_svm_regions, animate, 500);
}
...
function resize() {
    ...
    skc_width  = $('#d3-oscars-popular').width() - skc_margin.left - skc_margin.right;
    oscars_height = $('#d3-oscars-popular').height() - skc_margin.top - skc_margin.bottom;

    // rescale plot
    oscars_x.range([0, skc_width]);
