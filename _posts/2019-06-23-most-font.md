...
date: '2019-06-23T09:00:00.000-06:00'
thumbnail: /assets/img/post-thumbnails/most-font-d8e32c49.png
related-proj: most-font
---
...
<div class="columns two">
    ...
    <div class="column">
        {% include tweet.html url="https://twitter.com/zachlieberman/status/1077653990760951813" text="Love written with every font on my hard drive — happy holidays ! #openframeworks" author="@zachlieberman" %}
    </div>
</div>

**Note**: This post will contain some font design lingo, so if you’re relatively new to typography (like I am), you can read some basics on [these](http://typedia.com/learn/only/anatomy-of-a-typeface/) [pages](https://www.fonts.com/content/learning/fontology/level-1) [for](https://material.io/design/typography/understanding-typography.html) [background](http://www.happytypings.com/alphabet/).
...
All images and gifs were generated using a combination of [Processing](https://processing.org/) and [ImageMagick](https://www.imagemagick.org/).

	The first iteration of this project included all of the 712 fonts on my computer, but I decided to pare it down to the primary/”regular” versions of each font (e.g., keeping Helvetica, but not Helvetica Oblique or Helvetica Light), leaving a total of 255 fonts. A handful of these fonts were also primarily designed for non-Latin alphabets (for example, [Waseem](https://www.diwan.com/index.php/products/fonts/36-diwan-arabic-fonts) was designed for Arabic), but since I was analyzing characters in the Latin alphabet, Processing often displayed these fonts using standard fonts.

	When initially generating letter heatmaps, Processing would frequently render fonts <span id="footnote-1" class="footnote">at inconsistent sizes and with inconsistent offsets/positions</span>. As much as font sizes are supposed to be standardized to describe a font’s height in pixels, not all fonts at centered 100 pixels will actually render centered with a height of 100 pixels (some will be taller/shorter, some will have padding, etc.). To compensate for this, I calculated the [bounding box](https://computersciencewiki.org/index.php/Bounding_boxes) around each font/letter combination and used the dimensions of these bounding boxes to scale and translate everything back to the center at the intended height.

	You can find all of the code, data, and generated images from the project in [this repo](https://github.com/ben-tanen/most-font).
{% endcapture %}
{% include methodology-note.html content=methodology-note %}

<script>
d3.selectAll("#font-list-list li").on("click", function(event) {
...
<img src='/projects/most-font/img/stacked_H_old.png' width='100%' />
{% endcapture %}
{% include footnote-content.html id="footnote-1" content=footnote-footnote-1-content %}

{% capture footnote-footnote-2-content %}
Interestingly, in a response describing the high or low vertex, the responder captured the essence of most of these design decisions: That's just one of hundreds of type design decisions with endless amounts of gradual options.
{% endcapture %}
{% include footnote-content.html id="footnote-2" content=footnote-footnote-2-content %}

{% capture footnote-footnote-3-content %}
See the methodology section below.
{% endcapture %}
{% include footnote-content.html id="footnote-3" content=footnote-footnote-3-content %}
